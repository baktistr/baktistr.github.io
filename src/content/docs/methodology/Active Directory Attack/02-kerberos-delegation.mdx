...
> 💡 **If `MachineAccountQuota = 0`** and you don't already control a computer account, RBCD is blocked. Use [**Shadow Credentials**](/docs/methodology/active-directory-attack/01-permission-delegation#attack-genericall) instead — it abuses the same GenericAll/GenericWrite right via `msDS-KeyCredentialLink` and only needs ADCS in the forest.
