...
### Linux-Friendly GPO Enumeration

When you don't have PowerView, read the same ACLs from Kali:

```bash
# BloodHound collection picks up GPO control edges (GenericAll, WriteProperty, etc.)
bloodhound-python -u user -p pass -d domain.local -ns DC_IP -c All

# nxc BloodHound collection (same data, one command)
nxc ldap DC_IP -u user -p pass --bloodhound -c All --dns-server DC_IP

# Read a specific GPO's ACL with dacledit
dacledit.py domain.local/user:pass -target-dn 'CN={GPO-GUID},CN=Policies,CN=System,DC=domain,DC=local' -action read -dc-ip DC_IP

# List GPOs and their gpcfilesyspath
ldapsearch -x -H ldap://DC_IP -D "user@domain.local" -w pass \
  -b "CN=Policies,CN=System,DC=domain,DC=local" "(objectClass=groupPolicyContainer)" displayName gPCFileSysPath
```
