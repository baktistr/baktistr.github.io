---
title: "About Me"
date: 2023-01-22T10:41:25+07:00
draft: false
---

Hello there, My name is **Bakti Satria Adhityatama**.

Welcome to **Blog & Portfolio Website**. here I Write anything about **_Cyber Security_**, **_Data Science_**, and **_Artificial Intelligence_** related things.

🚩 I’m currently a full-time employee of [Telkom Indonesia](https://telkom.co.id/) as a data Scientist
\
👨‍👩‍👧 I’m a husband, and father of one adorable daughter
\
🌱 I’m currently learning data structures and algorithms while attempting to solve [leetcode](https://leetcode.com/baktistr/) problems 
\
🏫 I have BSc and MSc. in Electrical Engineering From [Institut Teknologi Bandung (ITB)](https://itb.ac.id/)
\
🤟 I have a big interest in Cyber Security & Data Science field


## Professional Works and Experiences

- 2014 - Student Intern at Lussa, a local software house company in Bandung

- 2015 - Student Intern at Central bank of Indonesia

- 2016 - Backend Developer at Meridian.id a local software house company in Bandung

- 2017-2019 - IT & Asset Database officer at Telkom Indonesia's Asset Management Center Unit

- 2020-2022 - Asset Insurance & Physical Security Engineer at Telkom Indonesia's Asset Management Center Unit

- 2023-now - Data Scientist at Telkom Indonesia' Department of Digital Business


## Research Interest

- ***Machine Learning and AI***: In late 2022, I worked part-time as a research assistant at Institut Teknologi Bandung. I participated in research on wearable device technology for vital sign monitoring using mobile phones for non-contact photoplethysmography (PPG) measurements. My role was primarily focused on deploying the mobile application and the API for the Deep Learning Model.


- ***Social Network Analysis***: In my Master's Degree Thesis, I evaluated the performance of various algorithms for graph sampling in social media networks. I proposed enhanced methods suitable for networks with specific properties. The results of the graph sampling were then used for in-depth Social Network Analysis (SNA) on Twitter.

- ***Internet of Things & Embedded Systems***: For my Bachelor's Degree Thesis, I developed a fully functional prototype of an Electronic Road Pricing (ERP) System. I also designed a web application to monitor the ERP prototype and was responsible for ensuring the seamless API communication between the server and the mobile application.

- ***Cyber Security and Privacy*** - This field is more than just an area of study for me; it's a passion.



## Why do I make this blog ?

In my free time, I engage in various activities, such as reading books, taking online courses, solving leetcode & kaggle problems, and exploring labs in hacking playgrounds like Hackthebox and Tryhackme. Given my hobbies and my role in the tech industry, I began blogging to validate and solidify my understanding of specific topics. As the saying goes **_If you can't explain it simply, you don't understand it well enough_**.

Currently, I maintain this blog to document everything I learn on my journey to achieving the OSCP certification. The primary resource for my learning is this list of [OSCP-Like machines on Hackthebox from TjNull](https://docs.google.com/spreadsheets/d/1dwSMIAPIam0PuRBkCiDI88pU3yzrqqHkDtBngUHNCw8/edit#gid=1839402159). I also use this platform to chronicle my side projects and endeavors in the **Data Science & AI** domains.

I relish challenges and continuously push myself towards achieving greater goals. Most of the knowledge I possess comes from self-teaching and exploration. I genuinely believe that writing is one of the most potent tools to reinforce the information in my mind. Moreover, it's my guiding principle to remain a **lifelong learner** and always seek profound knowledge.

I also curate this blog to express heartfelt gratitude to everyone who has indirectly aided me with their writings, guides, blogs, etc., especially when I encounter challenges. I hope this chain of knowledge continues unbroken.

Additionally, I maintain another non-technical blog on WordPress (written in Bahasa Indonesia) that captures my daily reflections. You can find it [here](https://baktistr.wordpress.com/).
